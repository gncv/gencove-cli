...
def test_data_presign_success(mocker, credentials, recording, vcr):
    """Test data being output to shell."""
    runner = CliRunner()
    mocker.patch(
        "gencove.command.explorer.data.presign.main.user_has_aws_in_path",
        return_value=True,
    )
    if not recording:
    ...
    # Explorer files dir
    res = runner.invoke(presign, ["e://users/me/cli_test_file.txt", *credentials])

    if not recording:
        mocked_get_credentials.assert_called_once()
        mocked_aws.assert_called_once()
    else:
        assert "https://gencove-explorer-" in res.output
...
@pytest.mark.vcr
@assert_authorization
def test_data_presign_no_permission(mocker, credentials):
...
def test_data_read_credentials_from_env(mocker, credentials):
    ...
    mocked_request_is_from_explorer = mocker.patch(
        "gencove.command.explorer.data.presign.main.request_is_from_explorer",
        return_value=True,
    )
    mocker.patch("gencove.command.explorer.data.presign.main.Presign.execute")
    os.environ["GENCOVE_USER_ID"] = uuid.uuid4().hex
    os.environ["GENCOVE_ORGANIZATION_ID"] = uuid.uuid4().hex

    runner.invoke(presign, ["e://users/me/", *credentials])
