...
@pytest.mark.vcr
@assert_authorization
def test_list_project_samples_include_hidden(
    mocker, credentials, project_id, recording, vcr
):  # pylint: disable=unused-argument
    """Test project samples being outputed to the shell."""
    runner = CliRunner()
    ...
    res = runner.invoke(
        list_project_samples,
        [project_id, "--hidden", *credentials],
    )
    assert res.exit_code == 0
    if not recording:
        mocked_get_project_samples.assert_called_once()
        samples = mocked_samples["results"]
        # at least one hidden sample
        assert any(sample["hidden"] for sample in samples)
        output_line = io.BytesIO()
        sys.stdout = output_line
        for mocked_sample in samples:
            mocked_sample = SampleDetails(**mocked_sample)
            echo_data(
                "\t".join(
                    [
